import { describe, it, expect } from 'vitest'
import { Brands } from '../Brands'

describe('Brands collection', () => {
  describe('Collection structure', () => {
    it('has correct slug', () => {
      expect(Brands.slug).toBe('brands')
    })

    it('uses name as display title', () => {
      expect(Brands.admin?.useAsTitle).toBe('name')
    })

    it('is in Taxonomy admin group', () => {
      expect(Brands.admin?.group).toBe('Taxonomy')
    })

    it('has timestamps enabled', () => {
      expect(Brands.timestamps).toBe(true)
    })
  })

  describe('Access control', () => {
    it('allows public read', () => {
      const result = Brands.access?.read?.({ req: {} } as any)
      expect(result).toBe(true)
    })

    it('blocks unauthenticated create', () => {
      const result = Brands.access?.create?.({
        req: { user: undefined },
      } as any)
      expect(result).toBe(false)
    })

    it('allows authenticated create', () => {
      const result = Brands.access?.create?.({
        req: { user: { id: '1' } },
      } as any)
      expect(result).toBe(true)
    })

    it('blocks unauthenticated update', () => {
      const result = Brands.access?.update?.({
        req: { user: undefined },
      } as any)
      expect(result).toBe(false)
    })

    it('allows authenticated update', () => {
      const result = Brands.access?.update?.({
        req: { user: { id: '1' } },
      } as any)
      expect(result).toBe(true)
    })

    it('blocks unauthenticated delete', () => {
      const result = Brands.access?.delete?.({
        req: { user: undefined },
      } as any)
      expect(result).toBe(false)
    })

    it('allows authenticated delete', () => {
      const result = Brands.access?.delete?.({
        req: { user: { id: '1' } },
      } as any)
      expect(result).toBe(true)
    })
  })

  describe('Fields', () => {
    it('has name field (text, required)', () => {
      const field = Brands.fields?.find((f: any) => f.name === 'name') as any
      expect(field).toBeDefined()
      expect(field?.type).toBe('text')
      expect(field?.required).toBe(true)
    })

    it('has slug field (text, unique, indexed, readOnly)', () => {
      const field = Brands.fields?.find((f: any) => f.name === 'slug') as any
      expect(field).toBeDefined()
      expect(field?.type).toBe('text')
      expect(field?.unique).toBe(true)
      expect(field?.index).toBe(true)
      expect(field?.admin?.readOnly).toBe(true)
    })

    it('has description field (textarea)', () => {
      const field = Brands.fields?.find(
        (f: any) => f.name === 'description',
      ) as any
      expect(field).toBeDefined()
      expect(field?.type).toBe('textarea')
    })

    it('has exactly 3 fields', () => {
      expect(Brands.fields).toHaveLength(3)
    })
  })

  describe('Slug generation hook', () => {
    it('generates lowercase slug from name', () => {
      const hook = Brands.hooks?.beforeChange?.[0]
      expect(hook).toBeDefined()
      if (!hook) return

      const result = hook({
        data: { name: 'Gucci' },
        operation: 'create',
      } as any)

      expect(result.slug).toBe('gucci')
    })

    it('replaces spaces with dashes and trims', () => {
      const hook = Brands.hooks?.beforeChange?.[0]
      if (!hook) return

      const result = hook({
        data: { name: '  Louis  Vuitton  ' },
        operation: 'create',
      } as any)

      expect(result.slug).toBe('louis-vuitton')
    })

    it('removes special characters', () => {
      const hook = Brands.hooks?.beforeChange?.[0]
      if (!hook) return

      const result = hook({
        data: { name: "Macy's & More!" },
        operation: 'create',
      } as any)

      expect(result.slug).not.toContain("'")
      expect(result.slug).not.toContain('&')
      expect(result.slug).not.toContain('!')
    })

    it('handles Devanagari text gracefully', () => {
      const hook = Brands.hooks?.beforeChange?.[0]
      if (!hook) return

      const result = hook({
        data: { name: 'रेसमी ब्रांड' },
        operation: 'create',
      } as any)

      expect(result.slug).toBeDefined()
    })
  })
})
